...
- (BOOL)respondsToSelector:(SEL)aSelector {
  //QSLog(NSStringFromSelector(aSelector) );
...
- (IBAction)updateResultView:sender {
	...
	[self reloadResultTable];
	[[resultController resultTable]
	       selectRowIndexes:[NSIndexSet indexSetWithIndex:selection]
...
- (id)externalSelection {
	if (defaultBool(@"QSUseGlobalSelectionForGrab") )
		...
		return [QSObject fileObjectWithArray:[[QSReg getMediator:@"QSFSBrowserMediators"] selection]];
}
...
- (void)selectIndex:(int)index {
	...
	else if (index >= [resultArray count]) selection = [resultArray count] -1;
	...
	if ([[resultController window] isVisible]) [resultController updateSelectionInfo];  
}
