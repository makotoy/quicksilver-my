...
- (QSObject *) dObject {
	...
	if (!dObject){
		id objAr = [oDict objectForKey:@"directArchive"];
		if (objAr) {
			dObject = [QSObject objectWithDictionary:objAr];
			if (dObject) {
...
- (QSObject *) iObject {
	...
	if (!iObject) {
		NSDictionary* indDesc = [oDict objectForKey:@"indirectArchive"];
		if (indDesc) {
			iObject = [QSObject objectWithDictionary:indDesc];
			if (iObject)
