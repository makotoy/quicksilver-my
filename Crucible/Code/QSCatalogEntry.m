...
+ (NSSet *)keyPathsForValuesAffectingValueForKey:(NSString *)key
{
    ...
    if ([key isEqualToString:@"deepObjectCount"]) {
        resPaths = [resPaths setByAddingObjectsFromArray:
                      [NSArray arrayWithObjects:@"contents", @"enabled", nil]];
    } else if ([key isEqualToString:@"currentEntry"]) {
        resPaths = [resPaths setByAddingObject:@"selection"];
    } else if ([key isEqualToString:@"self"]) {
        resPaths = [resPaths setByAddingObject:@"count"];
    }
    return resPaths;
}
...
- (void)dealloc
{
	...
	[indexDate release], indexDate = nil;
...
{
	if (children) {
		[info setObject:[children valueForKey:@"dictionaryRepresentation"]
                 forKey:kItemChildren];
    }
	return info;
}
...
- (NSDate *)lastScanDate
{
	return [[[NSFileManager defaultManager] attributesOfItemAtPath:[self indexLocation]
...
- (NSString *)type
{
	...
	NSString *title = [[NSBundle bundleForClass:[source class]]
                         safeLocalizedStringForKey:theID
                         value:theID
...
- (BOOL)isGroup
{
    return [[info objectForKey:kItemSource] isEqualToString:@"QSGroupObjectSource"];
}
...
- (NSInteger)state
{
	...
	if ([[info objectForKey:kItemSource] isEqualToString:@"QSGroupObjectSource"]) { 
		for (id child in [self deepChildrenWithGroups:NO leaves:YES disabled:YES]) {
...
- (BOOL)isEnabled
{
	if ([self isRestricted]) return NO;

	if ([self isPreset]) {
		NSNumber *value = nil;
		if ((value = [QSLib presetIsEnabled:self])) return [value boolValue];
		if ((value = [info objectForKey:kItemEnabled])) return [value boolValue];  
...
- (NSMutableDictionary *)info { return info; }
...
{	
	...
	for (i = 0; i < [anc count] - 1; i++) {
		NSInteger index = [[[anc objectAtIndex:i] children]
                               indexOfObject:[anc objectAtIndex:i + 1]];
		if (!p) {
...
- (NSString *)name
{
	...
	if (!name) name = [info objectForKey:kItemName];

	if ([ID isEqualToString:@"QSSeparator"]) return @"";

	if (!name) {
		name = [(bundle ? bundle : [NSBundle mainBundle])
                  safeLocalizedStringForKey:ID
                  value:ID
...
- (NSInteger) count { return [self deepObjectCount]; }
...
- (BOOL)indexIsValid
{
	...
	if (!indexDate) {
        [self setIndexDate:
          [[manager attributesOfItemAtPath:indexPath error:NULL] fileModificationDate]];
    }
...
- (NSArray *)scannedObjects
{
    ...
    @catch(id localException) {
		...
		if (0 && DEBUG) {
            NSRunAlertPanel(@"Scan Error", errorMessage, nil, nil, nil);
		} else {
            QSLog(errorMessage);
        }
...
{
  [self setContents:[self scannedObjects]];
}
...
- (NSArray *)contentsScanIfNeeded:(BOOL)canScan
{
	...
	if ([[info objectForKey:kItemSource] isEqualToString:@"QSGroupObjectSource"]) { 
		...
		if (!contents && canScan) return [self scanAndCache];
		return contents;
	}
}
