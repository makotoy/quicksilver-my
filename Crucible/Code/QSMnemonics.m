...
- (NSDictionary *)objectMnemonics{ return objectMnemonics; }
...
- (void)addAbbrevMnemonic:(NSString *)mnem forID:(NSString *)key relativeToID:(NSString *)above immediately:(BOOL)immediately
{
	...
	if (index!=NSNotFound){
		...
		if ([[recentMnemonics objectForKey:mnem]isEqualToString:key]||immediately) {
            //Put after topmost unless is most recently used
...
- (void) setWriteTimer
{
	if ([writeTimer isValid]) [writeTimer invalidate];
	[writeTimer release];
