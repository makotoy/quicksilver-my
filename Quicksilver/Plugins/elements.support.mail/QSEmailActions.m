...
- (QSObject *) composeEmailTo:(QSObject *)dObject withItem:(QSObject *)iObject sendNow:(BOOL)sendNow direct:(BOOL)direct{
	...
	QSBasicObject *resIObject=[iObject resolvedObject];
    NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
	NSString *from=[defaults objectForKey:@"QSMailActionCustomFrom"];
	if (![from length])from=nil;
    if ([resIObject containsType:QSFilePathType]){
        subject=[NSString stringWithFormat:[defaults objectForKey:@"QSMailActionFileSubject"],[resIObject name]];
        body=[[NSString stringWithFormat:[defaults objectForKey:@"QSMailActionFileBody"],[resIObject name]]stringByAppendingString:@"\r\r"];
        attachments=[resIObject arrayForType:QSFilePathType];
    } else if ([resIObject containsType:NSStringPboardType]){
	...
	if (direct){
		[[QSReg getClassInstance:@"QSDirectMailMediator"]
		   sendEmailTo:addresses from:from subject:subject body:body
		   attachments:attachments sendNow:sendNow];
		
	}else{
		[[QSMailMediator defaultMediator] sendEmailTo:addresses from:from
